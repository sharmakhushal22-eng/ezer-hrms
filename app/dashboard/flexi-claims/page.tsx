...
const S = {
  ...
  inp: { padding: '7px 10px', border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 12, background: TK.sunken, color: C.navy, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: 'inherit' } as React.CSSProperties,
  ...
  pri: { padding: '8px 16px', background: C.purple, color: TK.onAccent, border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'inherit' } as React.CSSProperties,
  sec: { padding: '7px 12px', background: TK.surface, color: C.navy, border: `1px solid ${C.border}`, borderRadius: 10, cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' } as React.CSSProperties,
  green: { padding: '7px 12px', background: C.green, color: TK.onAccent, border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'inherit' } as React.CSSProperties,
}
    ...
    <div>
        ...
        {!loading && filtered.map(c => {
          ...
          return (
            <div key={c.id} onClick={() => toggle(c.id, isPending)} style={{ display: 'grid', gridTemplateColumns: cols, borderBottom: `1px solid ${C.border}`, alignItems: 'center', background: isSel ? C.purpleBg : c.status === 'APPROVED' ? '#F6FEF9' : c.status === 'REJECTED' ? '#FEF6F6' : '#fff', cursor: isPending ? 'pointer' : 'default' }}>
              ...
              <div style={cell}>{billN > 0 && <span style={{ fontSize: 10, padding: '2px 6px', border: `1px solid ${C.border}`, borderRadius: 7, marginRight: 3 }}>B:{billN}</span>}{supN > 0 && <span style={{ fontSize: 10, padding: '2px 6px', border: `1px solid ${C.border}`, borderRadius: 7 }}>S:{supN}</span>}{!billN && !supN && <span style={{ color: C.muted }}>—</span>}</div>
              <div style={cell}><AiBadge flag={c.ai_flag} /></div>
              <div style={{ ...cell, display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                {isPending ? <>
                  <button title="Approve" onClick={() => approve([c.id])} style={{ padding: '3px 8px', fontSize: 11, background: C.greenBg, color: C.green, border: `1px solid #BBF7D0`, borderRadius: 7, cursor: 'pointer' }}></button>
                  <button title="Reject" onClick={() => { setRejectIds([c.id]); setRejectReason('') }} style={{ padding: '3px 8px', fontSize: 11, background: C.redBg, color: C.red, border: `1px solid #FCA5A5`, borderRadius: 7, cursor: 'pointer' }}></button>
                  <button onClick={() => openBills(c)} style={{ padding: '3px 8px', fontSize: 10, background: TK.surface, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 7, cursor: 'pointer' }}>Bills</button>
      ...
      {rejectIds.length > 0 && (
        <div style={{ ...S.card, border: `1px solid #FCA5A5` }}>
          ...
          {rejectIds.length === 1 && (() => {
            ...
            return (
              <div style={{ background: C.amberBg, border: `1px solid #FDE68A`, borderRadius: 10, padding: '9px 11px', marginBottom: 8 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: C.amber, marginBottom: 5 }}>Partial rejection (optional)</div>
      ...
      {viewFiles && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setViewFiles(null)}>
          <div style={{ background: TK.surface, borderRadius: 14, padding: 20, maxWidth: 560, width: '100%', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {viewFiles.files.map((f: any) => (
              <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', border: `1px solid ${C.border}`, borderRadius: 10, marginBottom: 6 }}>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => pick(d)} title={label ? `${label} on day ${d}` : `Day ${d}`}
              style={{
                aspectRatio: '1', border: `1px solid ${edge ? C.purple : inRange ? TK.brandEdge : C.border}`, borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit',
                fontSize: 13, fontWeight: edge ? 700 : 500, padding: 0, position: 'relative',
  ...
  return (
    <div style={{ maxWidth: 620 }}>
      <div style={S.card}>
        ...
        <div style={{ marginTop: 12, background: C.purpleBg, borderRadius: 10, padding: '9px 12px', fontSize: 11, color: C.purpleDark }}>Employees can submit bills only while the window is <b>Open</b>. {NO_INVOICE.join(' & ')} are auto-claimed (no bill needed) and auto-approved.</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        ...
        {requests.map(r => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', border: `1px solid ${C.border}`, borderRadius: 10, marginBottom: 6, background: TK.brandTint }}>
            <div style={{ flex: 1 }}>
