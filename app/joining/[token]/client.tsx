...
const PRI = '#7C3AED'
const S = {
  pg:  { background: '#F5F3FF', minHeight: '100vh', fontFamily: '"DM Sans","Segoe UI",sans-serif', color: '#1E1B4B' } as const,
  ...
  inp: (conf: FieldConf = 'normal') => ({
    ...
    boxSizing: 'border-box' as const, fontFamily: 'inherit', color: '#1E1B4B',
    background: conf === 'ai-high' ? '#F0FDF4' : conf === 'ai-verify' ? '#FFFBEB' : conf === 'manual' ? '#FEF2F2' : '#FAFAF8',
...
const AIBadge = ({ conf }: { conf: FieldConf }) => {
  ...
  const map = { 'ai-high': ['#D1FAE5','#065F46','AI ✓'], 'ai-verify': ['#FEF3C7','#92400E','AI – Verify'], 'manual': ['#FEE2E2','#991B1B','Fill Required'] }
        ...
        <div style={{ ...S.cd, textAlign: 'center', padding: '40px 30px' }}>
          ...
          <div style={{ background: '#F3F0FF', borderRadius: 10, padding: 16, marginBottom: 20, textAlign: 'left' }}>
            ...
            {['7-step joining form filled','EPF Form 11 + Form 2 signed','Gratuity Form F signed',`${POLICIES.length} policies accepted`,'Aadhaar OTP eSign done','PDFs sent to your email + HR Manager'].map(s => (
              <div key={s} style={{ fontSize: 12, color: '#059669', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>✓ {s}</div>
            ))}
          </div>
          <div style={{ background: '#FFFBEB', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#92400E', marginBottom: 16, lineHeight: 1.7 }}>
            ⏳ HR Manager will generate your Employee Code shortly.<br/>You'll receive an email with your EZER ESS login details.
          </div>
      ...
      <div style={{ maxWidth: 480, width: '100%', padding: 24 }}>
        ...
        <div style={S.cd}>
          <div style={{ background: '#EEEDFE', borderRadius: 8, padding: '10px 12px', marginBottom: 14, fontSize: 12, color: '#534AB7', lineHeight: 1.7 }}>
            <strong>Documents to be signed:</strong><br/>
            ✓ Joining form (7 steps) &nbsp;·&nbsp; ✓ EPF Form 11 + Form 2<br/>
            ✓ Gratuity Form F &nbsp;·&nbsp; ✓ {POLICIES.length} policy acknowledgements<br/><br/>
          ...
          {!otpSent ? (
            <button onClick={() => { setOtpSent(true); alert('OTP sent to your Aadhaar-linked mobile number') }} style={{ ...S.btnP, width: '100%' }}>
            ...
            <>
              ...
              <button onClick={submitForm} disabled={saving || otp.length < 4} style={{ ...S.btnP, width: '100%', background: '#059669', opacity: saving || otp.length < 4 ? .6 : 1, cursor: saving || otp.length < 4 ? 'not-allowed' : 'pointer' }}>
      ...
      <div style={{ ...S.pg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ maxWidth: 560, width: '100%', padding: 16 }}>
          <div style={{ textAlign: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 17, fontWeight: 600, color: '#1E1B4B' }}>Policy Acceptance</div>
            <div style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>{policyIdx + 1} of {POLICIES.length} policies</div>
            <div style={{ display: 'flex', gap: 4, justifyContent: 'center', marginTop: 8 }}>
              {POLICIES.map((_, i) => (
                <div key={i} style={{ width: 30, height: 4, borderRadius: 99, background: i < policyIdx ? PRI : i === policyIdx ? '#A78BFA' : '#EDE9FE', transition: 'background .3s' }}/>
              ))}
            </div>
          </div>
          <div style={{ ...S.cd, border: `1.5px solid ${PRI}` }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#1E1B4B', marginBottom: 10 }}>{pol.name}</div>
            <div
              onScroll={e => { const el = e.currentTarget; if (el.scrollTop + el.clientHeight >= el.scrollHeight - 8) setScrolled(true) }}
              style={{ height: 180, overflowY: 'auto', background: '#FAFAF8', borderRadius: 8, padding: '12px 14px', fontSize: 12, color: '#374151', lineHeight: 1.9, marginBottom: 12, border: '1px solid #EDE9FE' }}>
              {pol.body}
              <div style={{ height: 30 }}/>
              {!scrolled && !isAccepted && <div style={{ textAlign: 'center', color: '#9CA3AF', fontSize: 11, marginTop: 8 }}>↑ Scroll to bottom to enable accept</div>}
            </div>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: 18 }}>
        ...
        <div style={{ ...S.cd, borderLeft: `3px solid ${PRI}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 10, background: '#EEEDFE', color: '#534AB7', padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>🤖 AI Pre-filled</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, fontSize: 12, marginBottom: 12 }}>
            {[['Name', form.full_name], ["Father's Name", form.father_name || '—'], ['DOB', form.dob || '—'], ['Gender', form.gender || '—'], ['Aadhaar', form.aadhaar ? '****'+form.aadhaar.slice(-4) : '—'], ['PAN', form.pan || '—'], ['Bank', form.bank_name || '—'], ['IFSC', form.bank_ifsc || '—'], ['DOJ', form.doj || '—']].map(([l,v]) => (
              <div key={l}><div style={{ fontSize: 10, color: '#9CA3AF' }}>{l}</div><div style={{ fontWeight: 500, color: '#1E1B4B' }}>{v}</div></div>
            ))}
          </div>
          <div style={S.g2}>
        ...
        <div style={{ ...S.cd, borderLeft: '3px solid #1D4ED8' }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>EPF Form 2 — Nominee Declaration <span style={{ color: '#DC2626', fontSize: 11 }}>* Manual fill</span></div>
          <div style={S.g3}>
        ...
        {/* ESIC */}
        <div style={{ ...S.cd, background: '#FAFAF8' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 12, color: '#6B7280' }}>ESIC Form 1 — Employee Declaration</div>
            {form.bank_acc ? (
              <span style={{ fontSize: 10, background: '#ECFDF5', color: '#059669', padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>Not applicable (Gross &gt; ₹21,000)</span>
            ) : (
              <span style={{ fontSize: 10, background: '#FFFBEB', color: '#D97706', padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>Check after salary confirmation</span>
            )}
          </div>
        </div>
  ...
  const STEP_CONTENT: Record<number, React.ReactNode> = {
    0: (
      <div>
        ...
        <div style={{ display: 'flex', gap: 14, marginBottom: 16, alignItems: 'flex-start' }}>
          <div onClick={() => F('photo_done', true)} style={{ width: 80, height: 80, borderRadius: 10, background: form.photo_done ? '#ECFDF5' : '#FEF2F2', border: `2px dashed ${form.photo_done ? '#059669' : '#FCA5A5'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', cursor: 'pointer', flexShrink: 0, gap: 4, transition: 'all .15s' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4 }}>Passport Size Photo <span style={{ color: '#DC2626', fontSize: 12 }}>*Required</span></div>
            <div style={{ fontSize: 11, color: '#6B7280', lineHeight: 1.7 }}>Recent photo (within 3 months). White background preferred. JPG/PNG, max 2MB.</div>
            {!form.photo_done && <div style={{ fontSize: 11, color: '#DC2626', marginTop: 4 }}>← Click box to upload</div>}
          </div>
        </div>
        <div style={S.g2}>
    ...
    3: (
      <div>
        ...
        {form.bank_acc && form.bank_acc_confirm && form.bank_acc !== form.bank_acc_confirm && (
          <div style={{ color: '#DC2626', fontSize: 12, marginTop: -6 }}>⚠️ Account numbers don't match</div>
        )}
      </div>
    ),
    4: (
      <div>
        <div style={{ fontSize: 12, color: '#6B7280', marginBottom: 14, lineHeight: 1.7 }}>2 professional references required — previous manager, colleague, or mentor. Not family members.</div>
        {[1, 2].map(n => (
    ...
    5: (
      <div>
        ...
        <div style={{ fontSize: 13, fontWeight: 500, color: '#DC2626', marginBottom: 10 }}>New documents required today:</div>
        {[
          ...
          <div key={key} onClick={() => F(key, true)} style={{ border: `1.5px dashed ${required && !(form as any)[key] ? '#FCA5A5' : '#A7F3D0'}`, borderRadius: 10, padding: '12px 14px', marginBottom: 10, cursor: 'pointer', background: (form as any)[key] ? '#ECFDF5' : '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'all .15s' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 500 }}>{emoji} {label} {required && <span style={{ color: '#DC2626' }}>*</span>}</div>
    ...
    6: (
      <div>
        ...
        {[
          ...
          <div key={s} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: '#FAFAF8', borderRadius: 8, marginBottom: 6, border: '1px solid #EDE9FE' }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 500, color: '#1E1B4B' }}>{label}</div>
              <div style={{ fontSize: 11, color: '#6B7280', marginTop: 1 }}>{summary}</div>
            </div>
            <button onClick={() => setStep(s)} style={{ ...S.btnO, padding: '4px 10px', fontSize: 11 }}>✏️ Edit</button>
          </div>
        ))}
  ...
  return (
    <div style={S.pg}>
      ...
      <div style={{ display: 'flex', overflowX: 'auto', padding: '9px 14px', gap: 3, background: '#fff', borderBottom: '1px solid #EDE9FE' }}>
        {STEPS.map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            <div onClick={() => { if (i <= step) setStep(i) }} style={{ padding: '5px 11px', borderRadius: 99, fontSize: 11, fontWeight: 500, cursor: i <= step ? 'pointer' : 'default', whiteSpace: 'nowrap', transition: 'all .2s', background: i === step ? PRI : i < step ? '#EDE9FE' : '#FAFAFA', color: i === step ? '#fff' : i < step ? PRI : '#9CA3AF' }}>
              {i < step ? '✓ ' : `${i+1}. `}{s}
            </div>
            {i < STEPS.length - 1 && <div style={{ width: 10, height: 1, background: '#EDE9FE', flexShrink: 0 }}/>}
          </div>
        ))}
      </div>

      {/* AI banner */}
      <div style={{ background: '#ECFDF5', borderBottom: '1px solid #A7F3D0', padding: '7px 16px', fontSize: 11, color: '#065F46', display: 'flex', alignItems: 'center', gap: 6 }}>
        🤖 <span>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '16px 20px' }}>
        <div style={S.cd}>
          <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 14, color: '#1E1B4B' }}>Step {step + 1}: {STEPS[step]}</div>
          {STEP_CONTENT[step]}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
          ...
          {step < 6 ? (
            ...
            <button onClick={() => setPhase('compliance')} style={{ ...S.btnP, background: '#059669', padding: '10px 24px' }}>
