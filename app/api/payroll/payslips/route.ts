...
function staleReason(ctx: RunCtx): string | null {
  ...
  let older = 0, olderCfg = 0
  for (const s of ctx.snapshots) {
    const l = ctx.lines.get(s.employee_id)
    if (!l) continue
    if (s.synced_at && l.created_at && new Date(l.created_at) < new Date(s.synced_at)) older++
    if (ctx.configVersion && s.tds_config_version && new Date(s.tds_config_version) < new Date(ctx.configVersion)) olderCfg++
    if (ctx.configVersion && !s.tds_config_version) olderCfg++
  }
  if (older) return `${older} employee${older === 1 ? '' : 's'} were calculated before their Month Master was last synced — re-run payroll first so the payslip matches the engine.`
  if (olderCfg) return `${olderCfg} employee${olderCfg === 1 ? '' : 's'} were calculated under an older tax configuration — re-run payroll first.`
  return null
}
