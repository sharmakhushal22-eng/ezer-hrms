        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 14, maxWidth: 560 }}>
          ...
          {types.length === 0 ? <div style={{ fontSize: 12, color: C.muted }}>No active perquisite types — configure them in Configuration → Perquisite (run sql63/sql64 first).</div> : (
            <>
              ...
              <button onClick={saveAmounts} disabled={saving} style={{ marginTop: 12, padding: '8px 16px', borderRadius: 8, border: 'none', background: 'linear-gradient(120deg,#2563EB,#5B21B6)', color: '#fff', fontWeight: 700, fontSize: 12, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving…' : 'Save amounts'}</button>
            </>
          )}
        </div>
      )}
