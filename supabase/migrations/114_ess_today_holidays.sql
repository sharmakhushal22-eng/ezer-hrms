-- =============================================================================
-- 114_ess_today_holidays.sql
-- "Next holiday" read the wrong table. One function replaced.
--
-- FOR: Nayan Ahuja. NO SCHEMA CHANGE. SAFE TO RUN TWICE.
--
-- THE SYMPTOM
--
-- No error. The payload builds and next_holiday is simply always null, so the
-- card never appears — the failure mode that does not announce itself.
--
-- THE CAUSE, WHICH IS MINE
--
-- I mapped the drop's `holidays` table to holiday_calendar because the name
-- matched and it had from_date and name columns. It is the FY HEADER: one row,
-- "FY 2026-27 Holiday Calendar", spanning 1 Apr to 31 Mar. Read as a list of
-- holidays it describes a single holiday a year long, and since its from_date
-- is in the past the `>= current_date` filter excluded it every time.
--
-- The actual dates are in holiday_entries (calendar_id, holiday_date,
-- description, holiday_type, is_optional, on_weekly_off). The label column is
-- `description`, not `name`.
--
-- WHY THE EARLIER CHECK MISSED IT
--
-- After 112 I verified that all 76 columns the payload names exist. They did —
-- holiday_calendar.from_date and .name are both real. Existence is not meaning,
-- and a table whose name matches is the easiest kind to map wrongly. This one
-- was only caught by reading the values back and asking why a card was empty.
--
-- Two of the remaining sources cannot be checked the same way yet:
-- leave_balances and payroll_employee_snapshot are both empty, so the tiles
-- render zeroes that are correct-for-now and unverifiable until there is data.
--
-- DEPENDS ON 111, 112, 113.
-- =============================================================================
