-- =============================================================================
-- 113_ess_today_celebrations.sql
-- ess_today_payload() fails on its celebrations block. One function replaced.
--
...
--     ERROR:  column x.on_date does not exist
--
-- The whole tab 500s again. The payload is one jsonb_build_object, so a single
-- bad expression takes all sixteen keys with it.
--
-- THE CAUSE, WHICH IS MINE
--
-- The celebrations block aliased the date as "on" to match the JSON contract:
--
--     select ... c.on_date as "on", c.years from (...) c ...   -- inner
--     select jsonb_agg(x order by x.on_date) from ( ... ) x    -- outer
--
-- Once renamed, x has a column called "on" and no on_date, so the ORDER BY
-- referenced something that had stopped existing one level up. Postgres raises
-- this when the function runs, not when it is created — the same reason 111 and
-- 112 both installed cleanly and failed on first use.
--
-- THE FIX
--
-- The keys are now built with jsonb_build_object, so the sort column (on_date)
-- and the output name ('on') are separate things. Nothing else changes.
--
-- THREE ROUND TRIPS FOR ONE FUNCTION, AND WHY
--
-- 111 shipped, 112 fixed a type error, 113 fixes this. All three are runtime
-- faults in a function that installs without complaint, and I could not run
-- SQL to find them — I verified the grants, the lockdown and that no existing
-- table was touched, but the payload itself only executes when something calls
-- it. After this I checked every one of the 76 column references the function
-- names against the live schema: all present. What remained were two internal
-- aliasing and typing mistakes, both now corrected.
--
